...
def feature_importance_analysis(model, results):
    ...
    for i in range(min(5, len(feature_names))):
        print(f"  {i+1}. {feature_names[indices[i]]} (重要性: {importances[indices[i]]:.4f})")
...
def main():
    ...
    # 2. 使用模型进行预测
    predictitions = predict_with_new_data(model, n_samples=100)
    
    # 3. 分析预测结果
    signals = analyze_predictions(predictitions, threshold=0.0)
    
    # 4. 特征重要性分析
    feature_importance_analysis(model, results)
    
    # 5. 模拟交易
    trading_results = simulate_trading(predictitions, initial_capital=100000.0, top_n=10)
    
    # 6. 保存结果
    save_predictions(predictitions, signals)
    ...
    print(f"\n生成的文件:")
    print(f"  1. predictions.csv - 预测结果")
    print(f"  2. feature_importance.png - 特征重要性图 (如果安装了matplotlib)")
    
    print(f"\n下一步建议:")
    print(f"  1. 查看预测结果: cat predictions.csv")
